...
try {
    ...
    Upload-File "./dist/index.html" "/public_html/index.html"
    
    # Upload favicon.svg
    if (Test-Path "./dist/favicon.svg") {
        Upload-File "./dist/favicon.svg" "/public_html/favicon.svg"
    }
    
    # Upload vite.svg
    if (Test-Path "./dist/vite.svg") {
        Upload-File "./dist/vite.svg" "/public_html/vite.svg"
    }
    
    # Upload assets para raiz
    Write-Host "Enviando pasta assets para raiz..." -ForegroundColor Yellow
    $assetsPath = "./dist/assets"
    if (Test-Path $assetsPath) {
        $assetFiles = Get-ChildItem -Path $assetsPath -File
        $uploadCount = 0
        foreach ($file in $assetFiles) {
            if (Upload-File $file.FullName "/public_html/assets/$($file.Name)") {
                $uploadCount++
            }
        }
